...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    const users = JSON.parse(
      localStorage.getItem("condoconnect_users") || "[]"
    );
  ...
  const register = async (
    ...
    const newUser = {
      ...
      condominiumCode: userData.condominiumCode,
      cpf: userData.cpf,
      cnpj: userData.cnpj,
      address: userData.address,
      internalAddress: userData.internalAddress,
      password,
  ...
  return (
    <AuthContext.Provider
      value={{ user, login, logout, register, updateUser, isAuthenticated }}
    >
      {children}
    </AuthContext.Provider>
  );
